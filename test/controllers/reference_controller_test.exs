defmodule EmpiriApi.ReferenceControllerTest do
  defmodule SharedContext do
    ...
    using do
      quote do
        ...
        @valid_attrs %{authors: "Doug Mill,Andrew Wong", title: "Empiri!", link: "http://www.empiri.co"}
  ...
  defmodule CreateContext do
    ...
    test "#{@action}: wrong content-type header", %{conn: conn} do
      conn = conn |> put_req_header("content-type", "application/xml")
                  |> post("/publications/1/references")
    ...
    test "#{@action}: user not found", %{conn: conn} do
      conn = conn |> put_req_header("content-type", "application/json")
                  ...
                  |> post("/publications/1/references", Poison.encode!(%{reference: @valid_attrs}))
    ...
    test "#{@action}: publication not found", %{conn: conn, user: _user} do
      ...
      assert_raise Ecto.NoResultsError, fn ->
        post(conn,
             "/publications/1/references",
             Poison.encode!(%{reference: @valid_attrs}))
      end
    end
    ...
    test "#{@action}: creates and renders resource when data is valid", %{conn: conn, user: _user, publication: publication} do
      conn = conn |> put_req_header("content-type", "application/json")
                  |> put_req_header("authorization", "Bearer #{generate_auth_token(@user_params)}")
                  |> post("/publications/#{publication.id}/references", Poison.encode!(%{reference: @valid_attrs}))

      assert json_response(conn, 201)["reference"]["title"] == "Empiri!"
      assert (Repo.all(from a in Reference,
                      limit: 1,
                      order_by: [desc: a.id])
             |> List.first).link == "http://www.empiri.co"
    ...
    test "#{@action}: does not create resource and renders errors when data is invalid", %{conn: conn, user: _user, publication: publication} do
      conn = conn |> put_req_header("content-type", "application/json")
                  |> put_req_header("authorization", "Bearer #{generate_auth_token(@user_params)}")
                  |> post("/publications/#{publication.id}/references", Poison.encode!(%{reference: @invalid_attrs}))
  ...
  defmodule UpdateContext do
    ...
    test "#{@action}: wrong content-type header", %{conn: conn} do
      conn = conn |> put_req_header("content-type", "application/xml")
                  |> put("/publications/1/references/1")
      ...
      reference = Repo.insert! Reference.changeset(%Reference{}, @valid_attrs)
       ...
       assert_raise Phoenix.MissingParamError, fn ->
        conn |> put_req_header("content-type", "application/json")
             |> put_req_header("authorization", "Bearer #{generate_auth_token(@user_params)}")
             |> put("/publications/#{publication.id}/references/#{reference.id}", Poison.encode!(%{junk: "garbage"}))
      end
    end
    ...
    test "#{@action}: user not found", %{conn: conn, user: _user, publication: publication} do
      ...
      conn = conn |> put_req_header("content-type", "application/json")
                  ...
                  |> put("/publications/#{publication.id}/references/#{reference.id}", Poison.encode!(%{reference: %{first_name: "Bob"}}))
    ...
    test "#{@action}: does not update resource and renders errors when data is invalid", %{conn: conn, user: _user, publication: publication} do
      ...
      conn = conn |> put_req_header("content-type", "application/json")
            |> put_req_header("authorization", "Bearer #{generate_auth_token(@user_params)}")
            |> put("/publications/#{publication.id}/references/#{reference.id}", Poison.encode!(%{reference: @invalid_attrs}))
    ...
    test "#{@action}: reference not found", %{conn: conn, user: _user, publication: publication} do
      ...
      assert_raise Ecto.NoResultsError, fn ->
        put(conn, "/publications/#{publication.id}/references/12345", Poison.encode!(%{reference: @valid_attrs}))
      end
    end
      ...
      test "#{@action}: updates and renders chosen resource when data is valid and user is an admin", %{conn: conn, user: _user, publication: publication} do
        ...
        conn = conn |> put_req_header("content-type", "application/json")
                    |> put_req_header("authorization", "Bearer #{generate_auth_token(@user_params)}")
                    |> put("/publications/#{publication.id}/references/#{reference.id}", Poison.encode!(%{reference: %{link: "www.blah.com"}}))

        assert json_response(conn, 200)["reference"]["id"]
        assert Repo.get(Reference, reference.id).link == "www.blah.com"
      end
  end
  ...
  defmodule DeleteContext do
    ...
    test "#{@action}: wrong content-type header", %{conn: conn} do
      conn = conn |> put_req_header("content-type", "application/xml")
                  |> delete("/publications/1/references/1")
    ...
    test "#{@action}: user not found", %{conn: conn, user: _user, publication: publication} do
      ...
      conn = conn |> put_req_header("content-type", "application/json")
                  |> put_req_header("authorization", "Bearer #{generate_auth_token(auth_id: 678)}")
                  |> delete("/publications/#{publication.id}/references/#{reference.id}", Poison.encode!(%{reference: @valid_attrs}))
    ...
    test "#{@action}: deletes the record when user is an admin", %{conn: conn, user: _user, publication: publication, user_pub: _user_pub} do
      ...
      assert Repo.get(Reference, reference.id) == nil
    end
  end
end
